...
#define NORMALS_STRENGTH _OrganicLightData.w
	...
	half ComputeLight(float3 wpos, float3 normalWS, float scale) {
		...
		half n1 = SAMPLE_TEXTURE2D_LOD(_NoiseTex, sampler_LinearRepeat, hazeUV.zy, 0).r;
		half n2 = SAMPLE_TEXTURE2D_LOD(_NoiseTex, sampler_LinearRepeat, hazeUV.xz, 0).r;
		half n3 = SAMPLE_TEXTURE2D_LOD(_NoiseTex, sampler_LinearRepeat, hazeUV.xy, 0).r;
