...
#define RGI_BLENDS
	...
	half4 FragCopyMultiTaps (VaryingsRGI i) : SV_Target {
        UNITY_SETUP_INSTANCE_ID(i);
        UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i);
        i.uv     = UnityStereoTransformScreenSpaceTex(i.uv);

        half4 pixel = SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, i.uv);
		half4 pixelN = SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, i.uv - float2(1, 1) * _MainTex_TexelSize.xy);
        half4 pixelW = SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, i.uv + float2(1, 1) * _MainTex_TexelSize.xy);
        half4 pixelS = SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, i.uv - float2(1, -1) *  _MainTex_TexelSize.xy);
        half4 pixelE = SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, i.uv + float2(1, -1) * _MainTex_TexelSize.xy);
        return (pixel + pixelN + pixelW + pixelS + pixelE) * 0.2;
	}
	...
	half4 FragSpecular (VaryingsRGI input) : SV_Target {
        ...
        UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
        float2 uv = UnityStereoTransformScreenSpaceTex(input.uv);
        half3 specular = SAMPLE_TEXTURE2D_X(_GBuffer1, sampler_LinearClamp, uv).rgb;
