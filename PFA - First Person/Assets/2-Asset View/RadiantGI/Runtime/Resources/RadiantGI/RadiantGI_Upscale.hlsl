...
#define RGI_UPSCALE
	...
	half4 FragCompose (VaryingsRGI i) : SV_Target {
        ...
        indirect *= saturate(LUMA_MAX / (lumaIndirect + 0.001));
        ...
        #if _FORWARD_AND_DEFERRED
            // mixed mode: try to sample source colors from albedo gbuffer. If it's full black, sample from camera color image
            half3 albedo = SAMPLE_TEXTURE2D_X_LOD(_GBuffer0, sampler_PointClamp, uv, 0).rgb;
            if (all(albedo == 0)) {
                ...
                half3 specular = SAMPLE_TEXTURE2D_X_LOD(_GBuffer1, sampler_PointClamp, uv, 0).rgb;
            ...
            half3 specular = SAMPLE_TEXTURE2D_X(_GBuffer1, sampler_PointClamp, uv).rgb;

            // apply surface reflection color
            indirect = indirect * albedo + indirect * specular * SPECULAR_CONTRIBUTION;
        #endif

        indirect = max(indirect, 0);
