...
#define RGI_RAYCAST
    ...
    float3 GetViewSpacePosition(float4 uv, float rawDepth) {
        #if UNITY_REVERSED_Z
        ...
        #endif
        depth = 2.0 * depth - 1.0;
        float3 rayStart = ComputeViewSpacePosition(uv.zw, depth, unity_CameraInvProjection);
        return rayStart;
    }
	...
	half4 FragRaycast (VaryingsRaycast input) : SV_Target {
        ...
        //float rawDepth = GetRawDepth(input.uv.xy);
    	float rawDepth = GetDownscaledRawDepth(input.uv.xy);  // faster
		
         // exclude skybox
		if (IsSkyBox(rawDepth)) return 0;

        float3 rayStart = GetViewSpacePosition(input.uv, rawDepth);
        float2 pos = uv * SOURCE_SIZE;
        float3 normalWS, normalVS;
        GetViewAndWorldNormal(uv, normalVS, normalWS);
        half4 indirect = 0;

//      float screenNoise = dot(float2(171.0, 231.0), pos); // Alternate noise generator #1
...
//		float3 noises = float3(noise.xy, screenNoise);
        ...
        indirect.rgb *= saturate(INDIRECT_MAX_BRIGHTNESS / (luma + 0.001));
 	    indirect.rgb *= INDIRECT_INTENSITY * (luma > LUMA_THRESHOLD);
